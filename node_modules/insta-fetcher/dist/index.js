...
class igApi {
    ...
    formatSidecar(metadata) {
        ...
        if (graphql.__typename == 'GraphSidecar') {
            for (let i = 0; i < graphql.edge_sidecar_to_children.edges.length; i++) {
                if (graphql.edge_sidecar_to_children.edges[i].__typename == 'GraphVideo') {
                    links.push({
                        type: 'video',
                        url: graphql.edge_sidecar_to_children.edges[i].node.video_url,
                        dimensions: graphql.edge_sidecar_to_children.edges[i].node.dimensions,
                    });
                }
                else {
                    links.push({
    ...
    fetchPost(url) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const shortcode = (0, utils_1.shortcodeFormatter)(url);
                const graphql = (yield RequestHandler_1.IGFetch.get(`/p/${shortcode}/?__a=1`))
                    .data.graphql;
